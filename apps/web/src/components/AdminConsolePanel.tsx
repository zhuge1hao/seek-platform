...
type TabId = "system" | "account" | "users" | "audit" | "maintenance" | "connectors" | "debug";
const tabs: Array<{ id: TabId; label: string; icon: typeof Activity }> = [
  { id: "system", label: "系统状态", icon: Activity }, { id: "account", label: "账号安全", icon: KeyRound }, { id: "users", label: "账号管理", icon: Users }, { id: "audit", label: "操作日志", icon: FileClock }, { id: "maintenance", label: "配置维护", icon: Settings }, { id: "connectors", label: "本地 Agent", icon: Cable }, { id: "debug", label: "联调记录", icon: TerminalSquare }
];
...
export function AdminConsolePanel({ open, onClose, onChanged }: { open: boolean; onClose: () => void; onChanged?: () => void }) {
  ...
  return <><SafeDrawer open={open} title={tabs.find((item) => item.id === tab)?.label || "后台管理"} eyebrow="meizhaiseek v1.6.2" onClose={onClose} sidebar={sidebar}>
    ...
    {tab === "connectors" ? <AgentConnectorPanel /> : null}{tab === "debug" ? <DebugPayloadPanel /> : null}
