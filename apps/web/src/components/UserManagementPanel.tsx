...
import { createAdminUser, getAdminUserRuns, resetAdminUserPassword, setAdminUserEnabled, updateAdminUser, type AdminUser, type AgentRunSummary } from "@/lib/api";
...
export function UserManagementPanel() {
  ...
  const { data, error: loadError, isLoading, mutate } = useAdminUsers({ keyword: keyword || undefined, role: role || undefined, enabled: enabled || undefined });
  const users = (data?.users || []).filter((item) => item?.user_id && item?.username && item?.role);
  ...
  const load = useCallback(async () => { setError(""); await mutate(); }, [mutate]);
  ...
  const openEdit = (user: AdminUser) => { setSelected(user); setForm({ username: user.username, password: "", confirmPassword: "", role: user.role, enabled: user.enabled !== false, remark: user.remark || "" }); setMode("edit"); setError(""); };
  ...
  const save = async () => {
    ...
    if ((mode === "create" || mode === "password") && form.password.length < 8) return setError("密码至少需要 8 位。");
...
function EditorFooter({ busy, onCancel, onSave, label }: { busy: boolean; onCancel: () => void; onSave: () => void; label: string }) { return <div className="flex justify-end gap-2"><button className="rounded-xl border px-4 py-2 text-sm" onClick={onCancel} type="button">取消</button><button className="rounded-xl bg-violet-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={busy} onClick={onSave} type="button">{busy ? "处理中..." : label}</button></div>; }
