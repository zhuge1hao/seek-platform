...
import { getAgentConfigs, getAgentConnectors, updateAgentConfig, type AgentConfig, type AgentConnector } from "@/lib/api";
...
export function AgentConfigPanel({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved?: (configs: AgentConfig[]) => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getAgentConfigs(), getAgentConnectors()]).then(([result, connectorResult]) => {
      setConnectors(Array.isArray(connectorResult.items) ? connectorResult.items : []);
      const normalized = sanitizeAgentConfigs(result); setConfigs(normalized.configs);
      if (normalized.usedFallback) setError("后端智能体配置异常，当前已切换为前端安全默认配置。");
      const first = normalized.configs[0]; if (first) { setSelectedType(first.agent_type); setDraft(first); }
    }).catch(() => {
      const normalized = sanitizeAgentConfigs(null); setConfigs(normalized.configs); setError("后端智能体配置异常，当前已切换为前端安全默认配置。");
      if (normalized.configs[0]) { setSelectedType(normalized.configs[0].agent_type); setDraft(normalized.configs[0]); }
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const next = configs.map((item) => item.agent_type === updated.agent_type ? updated : item); setConfigs(next); setDraft(updated); setNotice("配置已保存。"); onSaved?.(next);
  ...
  return <SafeDrawer open={open} title={draft?.name || "智能体设置"} eyebrow={draft?.agent_type || "meizhaiseek v1.6.4"} onClose={onClose} sidebar={sidebar} footer={footer}>
