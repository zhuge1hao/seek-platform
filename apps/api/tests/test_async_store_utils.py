...
class AsyncStoreUtilsTest(unittest.TestCase):
    ...
    def test_concurrent_read_wrappers_use_thread_local_connections(self) -> None:
        ...
        async def read_many() -> list[object]:
        ...
        self.assertEqual(results[1]["run_id"], run["run_id"])
        self.assertEqual(results[2]["conversation_id"], conversation["conversation_id"])
        self.assertTrue(any(item["conversation_id"] == conversation["conversation_id"] for item in results[3]))
        self.assertEqual(results[4]["dataset_id"], dataset["dataset_id"])
        self.assertTrue(any(item["dataset_id"] == dataset["dataset_id"] for item in results[5]))
