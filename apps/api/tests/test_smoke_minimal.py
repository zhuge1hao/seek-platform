...
class SmokeMinimalTest(unittest.TestCase):
    ...
    def test_runtime_identity_requires_exact_version_and_model(self) -> None:
        smoke_minimal.assert_runtime_identity({"version": "v1.8.10", "model": "meizhaiseek 2.0"}, "v1.8.10", "meizhaiseek 2.0")
        with self.assertRaisesRegex(AssertionError, "version mismatch"):
            smoke_minimal.assert_runtime_identity({"version": "v1.8.7", "model": "meizhaiseek 2.0"}, "v1.8.10", "meizhaiseek 2.0")
        with self.assertRaisesRegex(AssertionError, "version is missing"):
            smoke_minimal.assert_runtime_identity({"model": "meizhaiseek 2.0"}, "v1.8.10", "meizhaiseek 2.0")
        with self.assertRaisesRegex(AssertionError, "model mismatch"):
            smoke_minimal.assert_runtime_identity({"version": "v1.8.10", "model": "wrong"}, "v1.8.10", "meizhaiseek 2.0")
